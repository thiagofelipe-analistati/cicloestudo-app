// ARQUIVO: frontend/src/components/SessaoEstudoModal/SessaoEstudoModal.tsx
...
export function SessaoEstudoModal({ 
  isOpen, 
  onRequestClose, 
  disciplinas, 
  ...
  onSessionSaved,
  revisaoContext,
}: SessaoEstudoModalProps) {
  const [formData, setFormData] = useState({
    data: getTodayString(), disciplinaId: '', topicoId: '',
    categoria: 'Estudo', tempoEstudado: '00:00:00',
    acertosQuestoes: 0, errosQuestoes: 0,
  });
  ...
  useEffect(() => {
    if (isOpen) {
      if (revisaoContext) {
        setFormData({
          data: getTodayString(),
          disciplinaId: revisaoContext.topico.disciplina.id,
          topicoId: revisaoContext.topico.id,
          categoria: 'Revisão',
          tempoEstudado: secondsToTime(initialTimeInSeconds),
          acertosQuestoes: 0,
          errosQuestoes: 0,
        });
      } else {
        setFormData({
          ...
          categoria: 'Estudo',
          tempoEstudado: initialTimeInSeconds > 0 ? secondsToTime(initialTimeInSeconds) : '00:30:00',
          acertosQuestoes: 0, errosQuestoes: 0,
        });
      }
  ...
  }, [isOpen, initialTimeInSeconds, revisaoContext]);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const dadosParaApi = {
      ...
      revisaoId: revisaoContext ? revisaoContext.id : undefined,
    };
    ...
    <Modal isOpen={isOpen} onRequestClose={onRequestClose} className={styles.modal} overlayClassName={styles.overlay}>
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div className={styles.formGroup}>
          ...
          <select name="disciplinaId" id="disciplinaId" value={formData.disciplinaId} onChange={handleChange} required disabled={!!revisaoContext}>
        ...
        <div className={styles.formGroup}>
          ...
          <select name="topicoId" id="topicoId" value={formData.topicoId} onChange={handleChange} disabled={!formData.disciplinaId || topicos.length === 0 || !!revisaoContext} required>
        ...
        {formData.topicoId && (
          <div className={styles.revisaoOptions}>
            <div className={styles.checkboxGroup}>
              <input type="checkbox" id="concluiuTopico" checked={concluiuTopico} onChange={(e) => {
            ...
            {concluiuTopico && (
              <div className={`${styles.checkboxGroup} ${styles.subOption}`}>
                <input type="checkbox" id="agendarRevisao" checked={agendarRevisao} onChange={(e) => setAgendarRevisao(e.target.checked)} />
        ...
        <div className={styles.formGroup}>
          ...
          <select name="categoria" id="categoria" value={formData.categoria} onChange={handleChange} disabled={!!revisaoContext}>
